...
/**
 * Ajax functions for moodleforum
 *
 * @module     mod/moodleforum
...
define(['jquery', 'core/ajax', 'core/templates', 'core/notification', 'core/config', 'core/url'],
    ...
    var t = {
        ...
        recordvote: function(discussionid, ratingid, userid, event) {
            var target = $(event.target).closest('.moodleforumpost').prev();
            var postid = target.attr('id');
            postid = postid.substring(1);

            var vote = ajax.call([{
                methodname: 'mod_moodleforum_record_vote',
                args: {
            ...
            vote[0].done(function(response) {
                ...
                if (ratingid === 2) {
                    parentdiv.children('a:first-of-type').children().attr(
                        'src', Url.imageUrl('vote/upvoted', 'moodleforum'));
                    parentdiv.children('a:nth-of-type(2)').children().attr(
                        'src', Url.imageUrl('vote/downvote', 'moodleforum'));
                } else if (ratingid === 1) {
                    parentdiv.children('a:first-of-type').children().attr(
                        'src', Url.imageUrl('vote/upvote', 'moodleforum'));
                    parentdiv.children('a:nth-of-type(2)').children().attr(
                        'src', Url.imageUrl('vote/downvoted', 'moodleforum'));
                } else {
                    parentdiv.children('a:first-of-type').children().attr(
                        'src', Url.imageUrl('vote/upvote', 'moodleforum'));
                    parentdiv.children('a:nth-of-type(2)').children().attr(
