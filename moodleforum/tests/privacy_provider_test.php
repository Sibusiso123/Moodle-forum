...
/**
 * Tests for the moodleforum implementation of the Privacy Provider API.
 *
...
use \mod_moodleforum\privacy\provider;
use mod_moodleforum\privacy\data_export_helper;

/**
 * Tests for the moodleforum implementation of the Privacy Provider API.
 *
 * @copyright  2018 Tamara Gunkel
 * @group mod_moodleforum
 * @group mod_moodleforum_privacy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_moodleforum_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    /**
     * @var \mod_moodleforum_generator Plugin generator
     */
    private $generator;
    ...
    public function setUp() {
        ...
        $this->generator = $this->getDataGenerator()->get_plugin_generator('mod_moodleforum');
    }
    ...
    public function test_user_has_never_posted() {
        // Create a course with moodleforum forums.
        ...
        $contextlist = $this->get_contexts_for_userid($user->id, 'mod_moodleforum');
        ...
        $this->export_context_data_for_user($user->id, $context, 'mod_moodleforum');
        ...
        $this->assertEmpty($writer->get_data([get_string('discussions', 'mod_moodleforum')]));
    ...
    public function test_user_has_never_posted_subscribed_to_forum() {
        ...
        \mod_moodleforum\subscriptions::subscribe_user($user->id, $forum, $context);
    ...
    public function test_user_has_posted_reply() {
        ...
        $discussion = $DB->get_record('moodleforum_discussions', ['id' => $discussion->id]);
        $otherdiscussion = $DB->get_record('moodleforum_discussions', ['id' => $otherdiscussion->id]);
        // The other discussion should not have been returned as we did not post in it.
        $this->assertEmpty($writer->get_data(data_export_helper::get_subcontext($otherdiscussion)));
    ...
    public function test_user_has_rated_others() {
        ...
        $forum = $this->getDataGenerator()->create_module('moodleforum', [
            'course' => $course->id,
            'scale'  => 100,
        ]);
        list($user, $otheruser) = $this->create_users($course, 2);
        ...
        $rating = array();
        $rating['moodleforumid'] = $forum->id;
        $rating['discussionid'] = $discussion->id;
        $rating['userid'] = $user->id;
        $rating['postid'] = $post->id;
        $rating['rating'] = 1;
        $this->getDataGenerator()->get_plugin_generator('mod_moodleforum')->create_rating($rating);
    ...
    public function test_user_tracking_data() {
        $course = $this->getDataGenerator()->create_course();
        $forumoff = $this->getDataGenerator()->create_module('moodleforum', ['course' => $course->id]);
        $cmoff = get_coursemodule_from_instance('moodleforum', $forumoff->id);
        $contextoff = \context_module::instance($cmoff->id);
        $forumon = $this->getDataGenerator()->create_module('moodleforum', ['course' => $course->id]);
        $cmon = get_coursemodule_from_instance('moodleforum', $forumon->id);
        list($user) = $this->create_users($course, 1);
        // Set user tracking data.
        \mod_moodleforum\readtracking::moodleforum_stop_tracking($forumoff->id, $user->id);
        \mod_moodleforum\readtracking::moodleforum_start_tracking($forumon->id, $user->id);
        // Run as the user under test.
        $this->setUser($user);
        ...
        $this->export_context_data_for_user($user->id, $contextoff, 'mod_moodleforum');
        $this->assertEquals(0,
    ...
    public function test_user_read_posts() {
        global $DB;
        $course = $this->getDataGenerator()->create_course();
        $forum1 = $this->getDataGenerator()->create_module('moodleforum', ['course' => $course->id]);
        $cm1 = get_coursemodule_from_instance('moodleforum', $forum1->id);
        $context1 = \context_module::instance($cm1->id);
        $forum2 = $this->getDataGenerator()->create_module('moodleforum', ['course' => $course->id]);
        $cm2 = get_coursemodule_from_instance('moodleforum', $forum2->id);
        $context2 = \context_module::instance($cm2->id);
        $forum3 = $this->getDataGenerator()->create_module('moodleforum', ['course' => $course->id]);
        $cm3 = get_coursemodule_from_instance('moodleforum', $forum3->id);
        $context3 = \context_module::instance($cm3->id);
        $forum4 = $this->getDataGenerator()->create_module('moodleforum', ['course' => $course->id]);
        $cm4 = get_coursemodule_from_instance('moodleforum', $forum4->id);
        ...
        $f1d1 = $DB->get_record('moodleforum_discussions', ['id' => $f1d1->id]);
        ...
        $f2d1 = $DB->get_record('moodleforum_discussions', ['id' => $f2d1->id]);
        ...
        $f3d1 = $DB->get_record('moodleforum_discussions', ['id' => $f3d1->id]);
        ...
        $f4d1 = $DB->get_record('moodleforum_discussions', ['id' => $f4d1->id]);
        list($f4d2, $f4p2) = $this->generator->post_to_forum($forum4, $author);
        // Insert read info.
        // User has read post1, but not the reply or second post in forum1.
        \mod_moodleforum\readtracking::moodleforum_add_read_record($user->id, $f1p1->id);
        // User has read post1 and its reply, but not the second post in forum2.
        \mod_moodleforum\readtracking::moodleforum_add_read_record($user->id, $f2p1->id);
        \mod_moodleforum\readtracking::moodleforum_add_read_record($user->id, $f2p1reply->id);
        // User has read post2 in forum3.
        \mod_moodleforum\readtracking::moodleforum_add_read_record($user->id, $f3p2->id);
        ...
        // Forum 1.
        $this->export_context_data_for_user($user->id, $context1, 'mod_moodleforum');
        ...
        $this->assertEmpty($readdata);
        // Forum 2.
        $this->export_context_data_for_user($user->id, $context2, 'mod_moodleforum');
        ...
        $this->assertEmpty($readdata);
        // Forum 3.
        $this->export_context_data_for_user($user->id, $context3, 'mod_moodleforum');
    ...
    public function test_post_attachment_inclusion() {
        ...
        $contextlist = $this->get_contexts_for_userid($author->id, 'mod_moodleforum');
        $this->assertCount(1, $contextlist);
        $this->export_context_data_for_user($author->id, $context, 'mod_moodleforum');
        $writer = \core_privacy\local\request\writer::with_context($context);
    ...
    public function test_all_users_deleted_from_context() {
        ...
        foreach ($posts as $post) {
            ...
            $forum = $forums[$discussion->moodleforum];
            $context = $contexts[$forum->id];
            // Mark the post as being read by user.
            \mod_moodleforum\readtracking::moodleforum_add_read_record($user->id, $post->id);
            // Rate the other users content.
        ...
        $discussionsinforum = array_filter($discussions, function ($discussion) use ($forum) {
            return $discussion->moodleforum == $forum->id;
        });
        ...
        $this->assertCount(0, $DB->get_records('moodleforum_discussions', ['moodleforum' => $forum->id]));
        list ($insql, $inparams) = $DB->get_in_or_equal(array_keys($discussionsinforum));
        $this->assertCount(0, $DB->get_records_select('moodleforum_posts', "discussion {$insql}", $inparams));
        // All uploaded files relating to this context should have been deleted (post content).
        foreach ($postsinforum as $post) {
            $this->assertEmpty($fs->get_area_files($context->id, 'mod_moodleforum', 'attachment', $post->id));
        }
        // All ratings should have been deleted.
        foreach ($postsinforum as $post) {
            $ratings = $DB->get_records('moodleforum_ratings', array('postid' => $post->id));
        ...
        $this->assertGreaterThan(0, $DB->count_records('moodleforum_discussions', ['moodleforum' => $forum->id]));
        list ($insql, $inparams) = $DB->get_in_or_equal(array_keys($discussionsinforum));
        $this->assertGreaterThan(0, $DB->count_records_select('moodleforum_posts', "discussion {$insql}", $inparams));
        // Uploaded files relating to this context should remain.
        foreach ($postsinforum as $post) {
            if ($post->parent == 0) {
                $this->assertNotEmpty($fs->get_area_files($context->id, 'mod_moodleforum', 'attachment', $post->id));
            }
        }
    ...
    public function test_delete_data_for_user() {
        ...
        foreach ($posts as $post) {
            ...
            \mod_moodleforum\readtracking::moodleforum_add_read_record($user1->id, $post->id);
        }
        ...
        $this->assertCount(4, $DB->get_records_select('moodleforum_posts',
        ...
        $this->assertCount(16, $DB->get_records('moodleforum_ratings', array('userid' => 0)));
    ...
    /**
     * Create courses and a moodleforum module for each course.
     *
    ...
    protected function create_courses_and_modules($count) {
        ...
        for ($i = 0; $i < $count; $i++) {
            $course = $this->getDataGenerator()->create_course();
            $forum = $this->getDataGenerator()->create_module('moodleforum', ['course' => $course->id]);
        }
    ...
    public function test_delete_data_for_users() {
        ...
        $moodleforums = [];
        $contexts = [];
        // Creates two moodleforums.
        for ($i = 0; $i < 2; $i++) {
            $moodleforum = $this->getDataGenerator()->create_module('moodleforum', [
                'course' => $course->id,
                'scale' => 100,
            ]);
            ...
            $contexts[$moodleforum->id] = $context;
        }

        $discussions = [];
        $posts = [];
        $postsbymoodleforum = [];
        // Foreach of the 5 users and foreach of the two moodleforums a post and discussion is created.
        ...
        $counter = 0;
        foreach ($users as $user) {
            $postsbymoodleforum[$user->id] = [];
            foreach ($moodleforums as $moodleforum) {
                $context = $contexts[$moodleforum->id];

                // Create a new discussion + post in the moodleforum.
                list($discussion, $post) = $this->generator->post_to_forum($moodleforum, $user);
                $discussion = $DB->get_record('moodleforum_discussions', ['id' => $discussion->id]);

                $discussions[$discussion->id] = $discussion;
                $postsbymoodleforum[$user->id][$context->id] = [];

                // Add a number of replies.
                $posts[$post->id] = $post;
                $thismoodleforumposts[$post->id] = $post;
                $postsbymoodleforum[$user->id][$context->id][$post->id] = $post;

                $reply = $this->generator->reply_to_post($post, $user);
                $posts[$reply->id] = $reply;
                $postsbymoodleforum[$user->id][$context->id][$reply->id] = $reply;
        ...
        foreach ($users as $user) {
            foreach ($posts as $post) {
                ...
                if ($post->userid != $user->id) {
                    ...
                    $DB->insert_record('moodleforum_ratings', $rating);
                }
            }
        }

        // Delete for one of the moodleforums all post for the first user.
        $firstcontext = reset($contexts);

        $approveduserlist = new \core_privacy\local\request\approved_userlist($firstcontext, 'mod_moodleforum', [$user1->id]);
        provider::delete_data_for_users($approveduserlist);

        // All posts should remain (5 user * 2 moodleforums * 4 Post per moodleforum).
        $this->assertCount(40, $DB->get_records('moodleforum_posts'));

        // User 1 posted in every moodleforum 4 post (8).
        // Post from the first moodleforum are made anonymous.
        $this->assertCount(4, $DB->get_records('moodleforum_posts', [
            'userid' => $user1->id,
        ]));

        // All of the post from moodleforum 1 should have been filed with empty values.
        ...
        // All ratings from user1 in moodleforum1 should have been deleted so 16 entries should be anonymous.
        // (20 post in moodleforum -4 post belonging to user 1).
        $this->assertCount(16, $DB->get_records_select('moodleforum_ratings', "userid = :userid"
           , ['userid' => 0 ]));
        // All ratings should still exist (2 moodleforums * 5 users * 16 post from other people).
        $this->assertCount(160, $DB->get_records('moodleforum_ratings'));

        // All discussions from user1 should be made anonymous (onyl one discussion created).
        // We stil have an entry for all 10 discussions (2 moodleforums * 1 discussion * 5 users).
        $this->assertCount(10, $DB->get_records('moodleforum_discussions'));

        // User 1 posted in every moodleforum 1 discussion.
        // Discussion from the first moodleforum are made anonymous.
        $this->assertCount(1, $DB->get_records('moodleforum_discussions', [
            'userid' => $user1->id,
        ]));

        // All of the discussions from moodleforum 1 should have been filed with empty values.
        // That means the userid equals 0, and name equals empty string.
        $this->assertCount(1, $DB->get_records_select('moodleforum_discussions', "userid = :userid"
        ...
        $deletedpostids = [];
        $otherpostids = [];
        foreach ($postsbymoodleforum as $user => $contexts) {
            foreach ($contexts as $thiscontextid => $theseposts) {
                $thesepostids = array_map(function($post) {
                    return $post->id;
                }, $theseposts);
    ...
    public function test_get_users_in_context_post_author() {
        ...
        $moodleforum = $this->getDataGenerator()->create_module('moodleforum', ['course' => $course->id]);
        $cm = get_coursemodule_from_instance('moodleforum', $moodleforum->id);
        $context = \context_module::instance($cm->id);

        list($author, $user) = $this->create_users($course, 2);

        list($fd1, $fp1) = $this->generator->post_to_forum($moodleforum, $author);

        $userlist = new \core_privacy\local\request\userlist($context, $component);
        \mod_moodleforum\privacy\provider::get_users_in_context($userlist);
    ...
    public function test_get_users_in_context_post_authors() {
        ...
        list($fd1, $fp1) = $this->generator->post_to_forum($moodleforum, $author);
        $fp1reply = $this->generator->post_to_discussion($moodleforum, $fd1, $user);
        $fd1 = $DB->get_record('moodleforum_discussions', ['id' => $fd1->id]);
    ...
    /**
     * Ensure that all users with a moodleforum subscription preference included as a user in the context.
     */
    public function test_get_users_in_context_with_subscription() {
        ...
        $othermoodleforum = $this->getDataGenerator()->create_module('moodleforum', ['course' => $course->id]);
        $othercm = get_coursemodule_from_instance('moodleforum', $othermoodleforum->id);
        $othercontext = \context_module::instance($othercm->id);

        list($user, $otheruser) = $this->create_users($course, 2);

        // Subscribe the user to the moodleforum.
    ...
    public function test_get_users_in_context_with_discussion_subscription() {
        ...
        // Post in both of the moodleforums.
        list($fd1, $fp1) = $this->generator->post_to_forum($moodleforum, $author);
        list($ofd1, $ofp1) = $this->generator->post_to_forum($othermoodleforum, $author);

        // Subscribe the user to the discussions.
        \mod_moodleforum\subscriptions::subscribe_user_to_discussion($user->id, $fd1, $context);
        \mod_moodleforum\subscriptions::subscribe_user_to_discussion($otheruser->id, $ofd1, $context);
    ...
    public function test_get_users_in_context_with_read_post_tracking() {
        ...
        \mod_moodleforum\readtracking::moodleforum_add_read_record($user->id, $fp1->id);
        \mod_moodleforum\readtracking::moodleforum_add_read_record($otheruser->id, $ofp1->id);
    ...
    public function test_get_users_in_context_with_tracking_preferences() {
        ...
        \mod_moodleforum\readtracking::moodleforum_stop_tracking($moodleforum->id, $user->id);
        \mod_moodleforum\readtracking::moodleforum_stop_tracking($othermoodleforum->id, $otheruser->id);

        $userlist = new \core_privacy\local\request\userlist($context, $component);
        \mod_moodleforum\privacy\provider::get_users_in_context($userlist);

        // One user - the one who is tracking that moodleforum.
        $this->assertCount(1, $userlist);
