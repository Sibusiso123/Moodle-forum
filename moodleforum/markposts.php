...
$moodleforumid = required_param('m', PARAM_INT);         // The moodleforuminstance to mark.
...
$url = new moodle_url('/mod/moodleforum/markposts.php', array('m' => $moodleforumid));
...
// Retrieve the connected moodleforum instance.
...
if ($returndiscussion === 0) {
    ...
    $returnto = new moodle_url("/mod/moodleforum/view.php", array('m' => $moodleforum->id));

} else {

    // Else relink back to the discussion we are coming from.
    $returnto = new moodle_url("/mod/moodleforum/discussion.php", array('d' => $returndiscussion));
}
...
if (isguestuser()) {
    ...
    // Create the message.
    $message = get_string('noguesttracking', 'moodleforum') . '<br /><br />' . get_string('liketologin');
...
if (!empty($discussionid)) {
    ...
    $options    = array('id' => $discussionid, 'moodleforum' => $moodleforum->id);
    $discussion = $DB->get_record('moodleforum_discussions', $options);
    if (!$discussion) {
        print_error('invaliddiscussionid', 'moodleforum');
    }

    // Mark all the discussions read.
    if (!\mod_moodleforum\readtracking::moodleforum_mark_discussion_read($discussionid, $user->id)) {

        // Display an error, if something failes.
        $message = get_string('markreadfailed', 'moodleforum');
        $status  = \core\output\notification::NOTIFY_ERROR;

    } else {

        // The discussion is successfully marked as read.
        $message = get_string('markmoodleforumreadsuccessful', 'moodleforum');
        $status  = \core\output\notification::NOTIFY_SUCCESS;
    }

    // Redirect the user.
    redirect(moodleforum_go_back_to($returnto), $message, null, $status);
    exit;

} else {

    // Mark all message read in the current instance.
    if (!\mod_moodleforum\readtracking::moodleforum_mark_moodleforum_read($cm, $user->id)) {
        ...
        $message = get_string('markdiscussionreadsuccessful', 'moodleforum');
        $status  = \core\output\notification::NOTIFY_SUCCESS;
    }
