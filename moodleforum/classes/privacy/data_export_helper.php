...
class data_export_helper {
    ...
    public static function export_discussion_data($userid, array $mappings) {
        ...
        $sql = "SELECT
                    d.*,
                    dsub.preference
                  FROM {moodleforum} mof
            INNER JOIN {moodleforum_discussions} d ON d.moodleforum = mof.id
            LEFT JOIN {moodleforum_discuss_subs} dsub ON dsub.discussion = d.id
        ...
        foreach ($discussions as $discussion) {
            $forumswithdata[$discussion->moodleforum] = true;
            $context = \context::instance_by_id($mappings[$discussion->moodleforum]);
    ...
    public static function export_all_posts($userid, array $mappings) {
        ...
        $sql = "SELECT
                  ...
                  FROM {moodleforum} mof
            INNER JOIN {moodleforum_discussions} d ON d.moodleforum = mof.id
            INNER JOIN {moodleforum_posts} p ON p.discussion = d.id
            LEFT JOIN {moodleforum_read} fr ON fr.postid = p.id
            LEFT JOIN {moodleforum_ratings} rat ON  rat.postid = p.id
     ...
     * Store all information about all posts that we have detected this user to have access to.
     * @param int       $userid     The userid of the user whose data is to be exported.
     * @param \context  $context    instance of the moodleforum context.
    ...
    protected static function export_all_posts_in_discussion($userid, \context $context, \stdClass $discussion) {
        ...
        $sql = "SELECT
              ...
              INNER JOIN {moodleforum_posts} p ON p.discussion = d.id
              LEFT JOIN {moodleforum_read} fr ON fr.postid = p.id AND fr.userid = :readuserid
              LEFT JOIN {moodleforum_ratings} rat ON rat.id = (
                SELECT id FROM {moodleforum_ratings} ra
        ...
        $discussionarea[] = get_string('posts', 'mod_moodleforum');
    ...
    protected static function export_post_data($userid, \context $context, $postarea, $post) {
        ...
        $postdata->message = writer::with_context($context)->rewrite_pluginfile_urls(
            $postarea, 'mod_moodleforum', 'attachment', $post->id, $post->message);
        ...
        writer::with_context($context)->export_data($postarea, $postdata)->export_area_files(
            $postarea, 'mod_moodleforum', 'attachment', $post->id);
    ...
    protected static function export_rating_data($postid, $onlyuser, $userid) {
        ...
        $ratingpost = $rating->moodleforum_get_rating($postid);
    ...
    public static function export_subscription_data(\stdClass $forum) {
        if (null !== $forum->subscribed) {
            ...
            writer::with_context(\context_module::instance($forum->cmid))->export_metadata(
                [], 'subscriptionpreference', 1, get_string('privacy:subscribedtoforum', 'mod_moodleforum'));
     ...
     *
     * @param \context_module $context      instance of the moodleforum context.
     * @param \stdClass       $discussion   The discussion whose data is being exported.
     *
     * @return bool         Whether any data was stored.
     */
    protected static function export_discussion_subscription_data(\context_module $context, \stdClass $discussion) {
        ...
        if (null !== $discussion->preference) {
            ...
            switch ($discussion->preference) {
                case \mod_moodleforum\subscriptions::moodleforum_DISCUSSION_UNSUBSCRIBED:
                    $a->preference = get_string('unsubscribed', 'mod_moodleforum');
                    break;
                default:
                    $a->preference = get_string('subscribed', 'mod_moodleforum');
                    break;
            }
            writer::with_context($context)->export_metadata(
                    ...
                    get_string('privacy:discussionsubscriptionpreference', 'mod_moodleforum', $a)
                );
    ...
    /**
     * Store moodleforum read-tracking data about a particular moodleforum.
     * This is whether a moodleforum has read-tracking enabled or not.
     *
     * @param \stdClass $forum  The moodleforum whose data is being exported.
     *
     * @return bool      Whether any data was stored.
     */
    public static function export_tracking_data(\stdClass $forum) {
        if (null !== $forum->tracked) {
            ...
            writer::with_context(\context_module::instance($forum->cmid))->export_metadata(
                [], 'trackreadpreference', 0, get_string('privacy:readtrackingdisabled', 'mod_moodleforum'));
    ...
    protected static function export_read_data(\context_module $context, array $postarea, \stdClass $post) {
        if (null !== $post->firstread) {
            ...
            writer::with_context($context)->export_metadata(
                    ...
                    get_string('privacy:postwasread', 'mod_moodleforum', $a)
                );
    ...
    protected static function get_discussion_area(\stdClass $discussion) {
        ...
        $pathparts[] = get_string('discussions', 'mod_moodleforum');
    ...
    /**
     * Get the parent subcontext for the supplied moodleforum, discussion, and post combination.
     *
    ...
    protected static function get_post_area_for_parent(\stdClass $post) {
        global $DB;
        $subcontext = [];
        if ($parent = $DB->get_record('moodleforum_posts', ['id' => $post->parent], 'id, created')) {
    ...
    /**
     * Get the subcontext for the supplied moodleforum, discussion, and post combination.
     *
    ...
    public static function get_subcontext($discussion = null, $post = null) {
        ...
        if (null !== $discussion) {
            ...
            if (null !== $post) {
                $subcontext[] = get_string('posts', 'mod_moodleforum');
