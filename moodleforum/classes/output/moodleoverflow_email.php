...
/**
 * moodleforum post renderable for e-mail.
 *
...
namespace mod_moodleforum\output;

defined('MOODLE_INTERNAL') || die();

/**
 * moodleforum email renderable for use in e-mail.
 *
 * @package   mod_moodleforum
 * @copyright 2017 Kennet Winter <k_wint10@uni-muenster.de>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class moodleforum_email implements \renderable, \templatable {

    /**
     * The course that the moodleforum post is in.
     *
     * @var object $course
     */
    ...
    /**
     * The course module for the moodleforum.
     *
     * @var object $cm
     */
    protected $cm = null;
    ...
    /**
     * The moodleforum that the post is in.
     *
     * @var object $moodleforum
     */
    protected $moodleforum = null;

    /**
     * The discussion that the moodleforum post is in.
     *
    ...
    /**
     * The moodleforum post being displayed.
     *
     * @var object $post
     */
    protected $post = null;
    ...
    /**
     * Builds a renderable moodleforum mail.
     *
     * @param object $course         Course of the moodleforum
     * @param object $cm             Course Module of the moodleforum
     * @param object $moodleforum The moodleforum of the post
     ...
     */
    public function __construct($course, $cm, $moodleforum, $discussion, $post, $author, $recipient, $canreply) {
        $this->course         = $course;
        $this->cm             = $cm;
        $this->moodleforum = $moodleforum;
     ...
     * Export this data so it can be used as the context for a mustache template.
     *
     * @param \mod_moodleforum_renderer $renderer The render to be used for formatting the message
     *
     * @return array Data ready for use in a mustache template
     */
    protected function export_for_template_text(\mod_moodleforum_renderer $renderer) {
        ...
        return array(
            ...
            'moodleforumname'            => html_entity_decode($this->get_moodleforumname()),
     ...
     * Export this data so it can be used as the context for a mustache template.
     *
     * @param \mod_moodleforum_renderer $renderer The render to be used for formatting the message and attachments
     *
     * @return stdClass Data ready for use in a mustache template
     */
    protected function export_for_template_html(\mod_moodleforum_renderer $renderer) {
        return array(
            'id'                            => $this->post->id,
            'coursename'                    => $this->get_coursename(),
            'courselink'                    => $this->get_courselink(),
            'moodleforumname'            => $this->get_moodleforumname(),
    ...
    public function get_unsubscribediscussionlink() {
        ...
        // Check whether the moodleforum is subscribable.
        $subscribable = \mod_moodleforum\subscriptions::is_subscribable($this->moodleforum);
        if (!$subscribable) {
            return null;
        }

        // Prepare information.
        $id  = $this->moodleforum->id;
        $d   = $this->discussion->id;
        $url = '/mod/moodleforum/subscribe.php';
    ...
    /**
     * ID number of the course that the moodleforum is in.
     *
     * @return string
     */
    ...
    /**
     * The full name of the course that the moodleforum is in.
     *
     * @return string
     */
    ...
    /**
     * The name of the course that the moodleforum is in.
     *
     * @return string
     */
    ...
    /**
     * The name of the moodleforum.
     *
     * @return string
     */
    public function get_moodleforumname() {
        return format_string($this->moodleforum->name, true);
    }

    /**
     * Whether to show the discussion name.
     * If the moodleforum name matches the discussion name, the discussion name is not typically displayed.
     *
     * @return boolean
     */
    public function has_showdiscussionname() {
        return ($this->moodleforum->name !== $this->discussion->name);
    }
    ...
    public function get_replylink() {
        return new \moodle_url(
            '/mod/moodleforum/post.php', array(
                'reply' => $this->post->id,
            )
        );
    }
    ...
    /**
     * Get the link to unsubscribe from the moodleforum.
     *
     * @return string
     */
    public function get_unsubscribemoodleforumlink() {
        if (!\mod_moodleforum\subscriptions::is_subscribable($this->moodleforum)) {
            return null;
        }
        $link = new \moodle_url(
            '/mod/moodleforum/subscribe.php', array(
                'id' => $this->moodleforum->id,
            )
        );
    ...
    protected function _get_discussionlink() {
        return new \moodle_url(
        // Posts are viewed on the topic.
            '/mod/moodleforum/discussion.php', array(
                // Within a discussion.
                'd' => $this->discussion->id,
            )
        );
    ...
    /**
     * Get the link to the moodleforum index for this course.
     *
     * @return string
     */
    public function get_moodleforumindexlink() {
        $link = new \moodle_url(
        // Posts are viewed on the topic.
            '/mod/moodleforum/index.php', array(
                'id' => $this->course->id,
            )
        );
    ...
    /**
     * Get the link to the view page for this moodleforum.
     *
     * @return string
     */
    public function get_moodleforumviewlink() {
        $link = new \moodle_url(
        // Posts are viewed on the topic.
            '/mod/moodleforum/view.php', array(
                'm' => $this->moodleforum->id,
            )
        );
    ...
    public function get_group_picture() {
        if (isset($this->userfrom->groups)) {
            $groups = $this->userfrom->groups[$this->moodleforum->id];
        } else {
